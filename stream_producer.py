...
Puts words into a selected stream, or numbers if no words are given.
If p is set, send words or numbers every p milliseconds.
...
""")
    ...
    parser.add_argument("-w", "--word", dest="words", default=[], action="append",
                        help="A word to add to the stream. Can be specified multiple times to "
                        "add multiple words.", metavar="WORD",)
    parser.add_argument("-p", "--period", dest="period", type=int,
                        help="If you'd like to repeatedly put words into the stream, this "
                        "option provides the period for putting words into the stream in "
                        "SECONDS. If no period is given then the words are put once.",
...
def put_words_in_stream(kinesis_client, stream_name, words, partition_key="123"):
    ...
    for w in words:
        try:
            kinesis_client.put_record(StreamName=stream_name, Data=w, PartitionKey=partition_key,
                                      SequenceNumberForOrdering=str(1))
            print("Put record '{}' into stream '{}'".format(w, stream_name))
        except Exception as e:
            print("Encountered an exception while trying to put record "
                  "'{}' into stream '{}'".format(w, stream_name))
...
def put_words_in_stream_periodically(kinesis_client, stream_name, words, period_seconds):
    ...
    # There is no strict guarantee about how frequently we put each word into the stream,
    # just that we will wait between iterations.
    while True:
...
def put_numbers_in_stream_periodically(kinesis_client, stream_name, period_seconds):
    ...
    # There is no strict guarantee about how frequently we put each number into the stream,
    # just that we will wait between iterations.
    n = 0
    while True:
